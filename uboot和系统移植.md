...
（3）shell程序执行时首先执行变量定义，然后执行主函数（主函数其实就是写在全局内的命令），其他函数在主函数代码中被调用执行。
